...
import {
    Box, Typography, Button, Grid, IconButton, Rating,
} from "@mui/material";
...
import { addToBagAsync } from '../Redux/slicers/bagSlice';
...
export default function ProductInfo() {
    ...
    useEffect(() => {
        const fetchProduct = async () => {
            try {
                const response = await axios.get(`${API_URL}/product/${category}/${_id}`);

                setProduct(response.data);
                setMainImage(response.data.images[0]);
            } catch (error) {
                toast.error("Error fetching product details", { position: "top-center" });
            }
        };
    ...
    if (!product) {
        return <Typography variant="h6" align="center" color="textSecondary">No Products Found.....</Typography>;
    }
        ...
        <Box sx={{ flexGrow: 1, padding: 2 }}>
            <Grid container spacing={2} alignItems="flex-start">
                    ...
                    <Box sx={{ display: "flex" }}>
                        ...
                        <Box sx={{ position: "relative", textAlign: "center" }}>
                            <Box component={"img"}
                                ...
                                alt={product.name}
                                sx={{
                                    width: { xs: "14rem", sm: "16.1rem", md: "22.7rem", lg: "24rem", xl: "32.4rem" },
                                    height: { xs: "18.2rem", sm: "20.5rem", md: "29.7rem", lg: "35rem", xl: "42rem" },
                                    borderRadius: "8px"
                                }}
                            />
                            <IconButton
                ...
                <Grid item xs={12} sm={6} md={6} lg={6} xl={6} >
                    <Box sx={{ paddingLeft: 2, paddingTop: 2, display: "flex", flexDirection: "column", }}>
