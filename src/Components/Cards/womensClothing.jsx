...
import {
  ...
  fetchWishlist,
} from "../Redux/slicers/wishlistSlice"; // Import wishlist actions

export default function WomensCards() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get(`${API_URL}/clothing/womens-clothing`);
    ...
    <Box sx={{ flexGrow: 1, padding: "0.4rem", marginTop: "0.5rem" }}>
      <Grid container rowSpacing={{ xs: 2, sm: 2, md: 3, lg: 4 }} columnSpacing={{ xs: 2, sm: 0, md: 0, lg: 0 }}>
        {products.slice(0, 8).map((product) => {
          const isFavorite = wishlistItems.some((item) => item.productId === product._id); // Check if product is in wishlist
