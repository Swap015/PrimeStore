...
export default function MensCards() {
    ...
    useEffect(() => {
        ...
        const fetchUserInfo = async () => {
            try {
                ...
                dispatch(fetchWishlist(response.data.user._id));
            } catch (error) {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                const response = await axios.get(`${API_URL}/clothing/mens-clothing`);
    ...
    return (
        <Box sx={{ flexGrow: 1, padding: "0.4rem", marginTop: "0.5rem", marginBottom: "0.9rem" }}>
                ...
                {products.slice(0, 8).map((product) => {
                    ...
                    return (
                        <Grid item xs={6} sm={4} md={3} lg={3} xl={3} key={product._id} sx={{ display: "flex", justifyContent: "center" }}>
                            <Card onClick={() => handleCardClick("clothing", product._id)} sx={{
